...
func _process(delta):	
	...
	if not ded:
		...
		if velocity.x == 0 and not Input.is_action_pressed("ui_down"):
			if is_on_floor() and not punching and not kicking and not highkicking:
	...
	if not ded:
		...
		if not is_on_floor() and Input.is_action_just_pressed("ui_kick"):
			if not punching and not kicking and not highkicking:
				highkicking = true
	...
	get_parent().get_node("CanvasLayer/TextureProgress2").value = vie
